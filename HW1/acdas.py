...
xv = np.ravel(xv[ind_rejection_method])
print(xv.shape)
k = num_pho//len(ind_rejection_method)+1
x_list = np.tile(xv,k)
...
while (True):
  ...
  xv = np.tile(xv, num_pho // len(xv) + 1)
  yv = np.tile(yv, num_pho // len(xv) + 1)
  Prob_xy = np.tile(yv, num_pho // len(xv) + 1)

  xv = xv[0:num_pho]
  yv = yv[0:num_pho]
  Prob_xy = Prob_xy[0:num_pho]
...
x_Prime = (1+D2/D1) * dx_list - x_list * D2/D1 # physical distance on CCD
y_Prime = (1+D2/D1) * dy_list - y_list * D2/D1 # physical distance on CCD

length_of_side = 10**-5 * u.m
center_CCD = (0+999)/2

x_CCD = np.round(-x_Prime/length_of_side+center_CCD) # 1000*1000 pixel coordinate
y_CCD = np.round(-y_Prime/length_of_side+center_CCD) # 1000*1000 pixel coordinate

my_dict = {"x_CCD": x_CCD, "y_CCD": y_CCD}
df = pd.DataFrame(my_dict, columns=['x_CCD','y_CCD'])
new_df = df.groupby(df.columns.tolist()).size().reset_index().rename(columns={0:'value'}) # 각 픽셀 광자가 몇 개 들어왔는가?
